...
from app.services.graph_builder import save_graph
...
def main():
    ...
    print(f"Starting BFS scrape from: {start_page} (Max depth: {max_depth}, Max neighbors/page: {max_neighbors})")
    ...
    try:
        while queue:
            ...
            if existing:
                print(f"Skipping '{current_title}' (already in DB). Loading links for queue...")
                if current_depth < max_depth:
                    links = session.query(Link).filter(Link.source_page_id == existing.page_id).all()
                    
                    added_count = 0
                    for link in links:
                        if added_count >= max_neighbors:
                            break
                        if link.target_title and link.target_title not in visited_in_run:
            ...
            try:
                ...
                existing_by_id = session.query(Page).filter(Page.page_id == node.page_id).first()
                if existing_by_id:
                    print(f"Page '{node.title}' (ID: {node.page_id}) already in DB. Updating...")
                ...
                if current_depth < max_depth:
                    ...
                    for edge in edges:
                        if added_count >= max_neighbors:
                            break
                        if edge.target_title and edge.target_title not in visited_in_run:
